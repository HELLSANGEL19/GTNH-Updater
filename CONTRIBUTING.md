...
```
GTNHUpdater/
  Update-GTNH.ps1              Entry point, version check, OS-level file lock, main dispatch
  ...
  lib/                         Module files (17 files)
    DisplayHelpers.ps1         Banner, colors, prompts, input helpers, cross-platform file manager, shared utilities (Remove-TempDir)
    ...
    NetworkApi.ps1             Downloads with progress/speed, API calls with ETag caching, integrity check, self-update
...
For sequential loops downloading multiple items, create a single HttpClient outside the loop and dispose it after:
...
try {
    foreach ($item in $items) {
        $bytes = $httpClient.GetByteArrayAsync($item.Url).Result
        # ... use $bytes ...
    }
} finally {
...
For parallel downloads, each parallel scriptblock must create its own HttpClient (scope isolation):
...
The script uses an OS-level file lock (`FileStream` with `FileShare.None`) instead of a PID-based lock file. This ensures the lock is automatically released when the process exits for any reason (clean quit, crash, terminal closed, killed). No stale lock files.
